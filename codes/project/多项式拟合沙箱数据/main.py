...
from functions import GetAllDaysOfOneMonth, FillnaRank, FillnaNewuser
from functions import FilterOutliers, FittingRevised

working_space = '/Users/Alas/Documents/TD/iOS_Rank_Estimated_Model_V1/jupyter_sandbox_data/project/'
path_to_write_fitting_data = working_space + '多项式拟合沙箱数据/fitting_data/'
path_to_write_fitting_coef = working_space + '多项式拟合沙箱数据/fitting_coef/'
...
#    print df_rank_newuser_revised


"""
    # 返回拟合值和拟合满意的参数(并按日期存入不同的文件夹)
    # final 来逐步调参达到想要的效果(不同多项式项数下的最小二乘法)
    num_coef = 80
    list_fitting_data = [-1,]

    while(min(list_fitting_data)<0):
        dict_return = FittingRevised(df_rank_newuser_revised, num_coef)
        list_fitting_data = dict_return['list_fitting_data']
        num_coef -= 1
"""


#    list_fitting_rank = dict_return['list_fitting_rank']
#    list_fitting_data = dict_return['list_fitting_data']
    #print len(range(list_fitting_rank[0], list_fitting_rank[1])), len(list_fitting_data)

#    for i,j in zip(range(list_fitting_rank[0], list_fitting_rank[1]), list_fitting_data):
#        print day, i, j
